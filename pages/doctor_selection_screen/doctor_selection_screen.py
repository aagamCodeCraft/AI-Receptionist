...
import os
from data.hospital_data import DOCTORS # Correctly import from the central file
...
class DoctorSelectionScreen(tk.Frame):
    ...
    def create_doctor_profile(self, parent, doctor_info):
        ...
        tk.Label(frame, text=doctor_info["name"], font=("Helvetica", 16, "bold"), bg="white", anchor="w").grid(row=0, column=1, sticky="w")
        tk.Label(frame, text=doctor_info["details"], font=("Helvetica", 11), bg="white", anchor="w", justify="left").grid(row=1, column=1, sticky="w", pady=(0, 10))
        ...
        create_detail_row("timings", doctor_info["timings"], 3)
        create_detail_row("fees", lang.get("consultation_fee").format(fees=doctor_info["fees"]), 4)
        create_detail_row("holidays", lang.get("holidays").format(holidays=doctor_info["holidays"]), 5)
